...
g{1}=[ sta '0060_07o.l12'];
g{2}=[ sta '0070_07o.l12'];
g{3}=[ sta '0080_07o.l12'];
g{4}=[ sta '0090_07o.l12'];
g{5}=[ sta '0100_07o.l12'];

r{1}=[ sta '0060_07o.rio'];
r{2}=[ sta '0070_07o.rio'];
r{3}=[ sta '0080_07o.rio'];
r{4}=[ sta '0090_07o.rio'];
r{5}=[ sta '0100_07o.rio'];
...
figure;
plot(res(:,1)/86400+6,res(:,2),'r','linewidth',2);
hold on
plot(res(:,1)/86400+6,res(:,4),'.');
grid on;
ylabel('Ionospheric delay (TECU)');
xlabel('DOY (2007)');
title(['Station ' sta ' - 2007 DOY 006 to 010']);
figname=['withIGS_' sta '2007'];
...
title(['Station ' sta ' - 2007 DOY 006 to 010']);
subplot(3,1,2);
scatter(re(:,1)/86400+6,re(:,9)/0.162372447511995,10,re(:,7));
grid on;
ylabel('Slant Residuals (TECU)');
xlabel('DOY (2007)');
subplot(3,1,3);
scatter(re(:,1)/86400+6,re(:,9)/0.162372447511995.*sin(re(:,8)),10,re(:,7));
grid on;
ylabel('Vertical Residuals (TECU)');
%ylim([-4 4])
xlabel('DOY (2007)');
figname=['residuals_' sta '2007'];
saveas(gcf,[figname '.emf']);
saveas(gcf,[figname '.fig']);
close(gcf)
...
for iday=1:5
    ...
    tit=['Station ' sta ' - 2007 DOY 00' num2str(iday)];
    title(tit);
    xlabel('Slant residuals (TECU)');
    ylabel('Occurrences');
    figname=['histslant_' sta '2007' num2str(iday)];
    saveas(gcf,[figname '.emf']);
    saveas(gcf,[figname '.fig']);
    close(gcf)
    figure;
    ...
    ylabel('Occurrences');
    figname=['histvert_' sta '2007' num2str(iday)];
    saveas(gcf,[figname '.emf']);
    saveas(gcf,[figname '.fig']);
    close(gcf)
end
...
tit=['Station ' sta ' - 2007 DOY 006 to 010'];
title(tit);
xlabel('Slant residuals (TECU)');
ylabel('Occurrences');
figname=['histslant_' sta '2007'];
saveas(gcf,[figname '.emf']);
saveas(gcf,[figname '.fig']);
close(gcf)
figure;
